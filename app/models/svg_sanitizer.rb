...
class SvgSanitizer
  ...
  private_class_method def self.remove_dangerous_attributes(content)
    # on*イベントハンドラを削除
    content.gsub(/\s+on\w+\s*=\s*["'][^"']*["']/i, "")
  end

  sig { params(content: String).returns(String) }
  private_class_method def self.remove_external_references(content)
