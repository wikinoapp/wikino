...
class ExportStatusRepository < ApplicationRepository
  ...
  def to_model(export_status_record:)
    ExportStatus.new(
      ...
      space: SpaceRepository.new.to_model(space_record: export_status_record.space_record),
      export: ExportRepository.new.to_model(export_record: export_status_record.export_record)
    )
  end
end
