...
cleanup() {
  ...
  # ラッパーPIDをwaitする前にポートを解放する。`op run -- go run` は
  # SIGTERMをコンパイル済みの子プロセスに伝播しないため、子がポートを保持し続け
  # ラッパーも終了しない。その状態でラッパーPIDを素朴にwaitするとハングする。
  # 先に実際のポート保持プロセスをkillするとラッパーが終了でき、下のwaitは
  # ハングせずにそれを回収できる。
