...
func TestRiverWorkerMiddleware_NoErrorDoesNotSendEvent(t *testing.T) {
	...
	err := mw.Work(ctx, newJobRow("send_email_confirmation", 1), func(ctx context.Context) error {
		return nil
	})
	if err != nil {
		t.Fatalf("Work()で予期しないエラー: %v", err)
	}

	hub.Flush(2 * time.Second)
	if got := len(transport.Events()); got != 0 {
		t.Errorf("成功ジョブのSentryイベントの件数 = %d、期待値 = 0", got)
	}
}
...
func TestRiverWorkerMiddleware_ErrorCapturesEvent(t *testing.T) {
	...
	mw := RiverWorkerMiddleware()
	err := mw.Work(ctx, newJobRow("send_password_reset", 3), func(ctx context.Context) error {
		return jobErr
	})
	if !errors.Is(err, jobErr) {
		t.Fatalf("Work() = %v、期待値 = %v (riverのリトライ判断のためそのまま伝搬する)", err, jobErr)
	}

	hub.Flush(2 * time.Second)
	events := transport.Events()
	if len(events) != 1 {
		t.Fatalf("失敗ジョブのSentryイベントの件数 = %d、期待値 = 1", len(events))
	}
	got := events[0]
	if len(got.Exception) != 1 || got.Exception[0].Value != "ジョブ失敗" {
		t.Errorf("event.Exception = %+v、期待値 = 値が%qの例外", got.Exception, "ジョブ失敗")
	}
	if got.Tags["job.kind"] != "send_password_reset" {
		t.Errorf("event.Tags[job.kind] = %q、期待値 = %q", got.Tags["job.kind"], "send_password_reset")
	}
...
func TestRiverWorkerMiddleware_DropsIgnorableErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mw := RiverWorkerMiddleware()
			err := mw.Work(ctx, newJobRow("cleanup_rate_limits", 1), func(ctx context.Context) error {
				return tt.err
			})
			if !errors.Is(err, tt.err) {
				t.Fatalf("Work() = %v、期待値 = %v (riverのリトライ判断のためそのまま伝搬する)", err, tt.err)
			}

			hub.Flush(2 * time.Second)
			if got := len(transport.Events()); got != 0 {
				t.Errorf("無視対象のエラーのSentryイベントの件数 = %d、期待値 = 0", got)
			}
		})
	}
}
...
func TestRiverWorkerMiddleware_BindsHubToContext(t *testing.T) {
	...
	// (sentryslog経由でctxのHubを使う)。本テストではinnerで
	...
	mw := RiverWorkerMiddleware()
	err := mw.Work(ctx, newJobRow("send_email_confirmation", 2), func(ctx context.Context) error {
		jobHub := sentry.GetHubFromContext(ctx)
		if jobHub == nil {
			t.Fatal("inner ctxにHubがbindされていない")
		}
		if jobHub == parentHub {
			t.Error("ジョブ単位のHubはCloneである必要がある (parentと同一インスタンスはNG)")
		}
	...
	events := transport.Events()
	if len(events) != 1 {
		t.Fatalf("innerからのCaptureExceptionの件数 = %d、期待値 = 1", len(events))
	}
	got := events[0]
	if len(got.Exception) != 1 || got.Exception[0].Value != "中間エラー" {
		t.Errorf("event.Exception = %+v、期待値 = 値が%qの例外", got.Exception, "中間エラー")
	}
	if got.Tags["job.kind"] != "send_email_confirmation" {
		t.Errorf("event.Tags[job.kind] = %q、期待値 = %q", got.Tags["job.kind"], "send_email_confirmation")
	}
