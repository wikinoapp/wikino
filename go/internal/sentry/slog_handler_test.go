...
// slog → sentryslog → beforeSendのパイプラインをEnd-to-Endで検証できるため、
// sentryslogのTagマッピング挙動が将来のバージョンアップで変わった場合に
// テストで検知できる。
...
func TestSlogHandler_ErrorIsCapturedToSentry(t *testing.T) {
	...
	if len(got.Exception) != 1 || got.Exception[0].Value != "disk full" {
		t.Errorf("値が%qの例外が無い: %+v", "disk full", got.Exception)
	}

	recs := base.Snapshot()
	if len(recs) != 1 {
		t.Fatalf("base recordの件数 = %d、期待値 = 1", len(recs))
	}
	if recs[0].level != slog.LevelError || recs[0].msg != "operation failed" {
		t.Errorf("base record = %+v、期待値 = Error/operation failed", recs[0])
	}
}
...
func TestSlogHandler_InfoIsNotCapturedToSentry(t *testing.T) {
	...
	if got := len(transport.Events()); got != 0 {
		t.Errorf("InfoレベルでのSentryイベントの件数 = %d、期待値 = 0", got)
	}
	...
	if recs[0].level != slog.LevelInfo || recs[0].msg != "request received" {
		t.Errorf("base record = %+v、期待値 = Info/request received", recs[0])
	}
}
...
func TestSlogHandler_BaseHandlerReceivesAllLevels(t *testing.T) {
	...
	if len(recs) != len(want) {
		t.Fatalf("base recordの件数 = %d、期待値 = %d", len(recs), len(want))
	}
	for i, w := range want {
		if recs[i].level != w.level || recs[i].msg != w.msg {
			t.Errorf("base record[%d] = (%v, %q)、期待値 = (%v, %q)", i, recs[i].level, recs[i].msg, w.level, w.msg)
		}
	}
}
...
func TestSlogHandler_DropsReverseProxySourceEventsEndToEnd(t *testing.T) {
	...
	// sentryslogがslog属性をevent.Tagsに乗せる挙動 (本判定の前提) が
	// 将来変わった場合に本テストで検知できる。
	...
	hub.Flush(2 * time.Second)
	if got := len(transport.Events()); got != 0 {
		t.Errorf("source=%sのイベントがbeforeSendでdropされていない: イベントの件数 = %d", ReverseProxySource, got)
	}
	...
	if recs[0].attrs[SourceAttrKey] != ReverseProxySource {
		t.Errorf("base attrs[%s] = %q、期待値 = %q", SourceAttrKey, recs[0].attrs[SourceAttrKey], ReverseProxySource)
	}
}
...
func TestSlogHandler_KeepsOtherSourceEventsEndToEnd(t *testing.T) {
	...
	events := transport.Events()
	if len(events) != 1 {
		t.Fatalf("source=otherのイベントがSentryに届いていない: イベントの件数 = %d", len(events))
	}
...
func TestSlogHandler_WithAttrs_PropagatesToBothHandlers(t *testing.T) {
	...
	if recs[0].attrs["request_id"] != "r123" {
		t.Errorf("base attrs[request_id] = %q、期待値 = %q", recs[0].attrs["request_id"], "r123")
	}
	if recs[0].attrs["user_id"] != "u456" {
		t.Errorf("base attrs[user_id] = %q、期待値 = %q", recs[0].attrs["user_id"], "u456")
	}
}
