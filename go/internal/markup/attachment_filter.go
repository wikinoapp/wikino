...
// AttachmentFinderは添付ファイルの検索インターフェース。
// repository.AttachmentRepositoryがこのインターフェースを満たす。
...
// FilterAttachmentsはHTML内の添付ファイルURLを持つimg・a要素を変換する。
...
// processANodeはa要素の添付ファイルURLを変換する。
...
// replaceWithInlineImageはimg要素をリンク付きインライン画像に置換する。
...
// downloadLinkSVGはダウンロードリンクのSVGアイコン
...
// replaceWithDownloadLinkはimg要素をダウンロードリンクに置換する。
...
// replaceWithInlineVideoはa要素をインライン動画に置換する。
...
// replaceWithAnchorDataAttrsはa要素の内容を保持しつつdata属性付きリンクに変換する。
...
// extractAttachmentIDはURLから添付ファイルIDを抽出する。
...
// WrapStandaloneImageLinksは<p>要素で囲まれていない独立した
...
// wrapImageLinksInNodeはノードの子要素を走査し、
...
// isAttachmentImageLinkはノードがa.wikino-attachment-image-link要素かを判定する
...
// getImageCaptionSiblingsは画像リンクの直後にキャプションパターン (<br> + <em>/<strong>) が
...
// wrapInParagraphWithSiblingsはノードと後続のキャプションノード群をまとめて<p>要素で囲む
...
// fileExtensionはファイル名から拡張子を小文字で取得する (ドット無し)
