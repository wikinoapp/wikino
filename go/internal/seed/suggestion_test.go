...
func TestGenerateSuggestions(t *testing.T) {
	...
	// 一覧には、まだ決着していない編集提案のタブと決着済みのタブがあり、決着済みの
	// タブは2つのステータスを持つ。3つとも作られている必要がある。そうでないと、
	// タブが空のまま、あるいは編集提案の終わり方2つのうち片方しか出ていない状態で
	// 眺めることになる。
	...
	for _, want := range []struct {
		...
		if got := byStatus[want.status]; got != want.count {
			t.Errorf("%sが%d件であることを期待したが%d件だった", want.name, want.count, got)
		}
	}

	// メンバーが編集提案に対して何をできるかは、それを自分が開いたかどうかで
	// 変わるため、2つのアカウントで交互に開く。期待するアカウントを
	// suggestionCreatorから求めないのは、選択処理の退行によって生成結果と期待値が
	// 同時に変わることを避けるため。
	for _, suggestion := range suggestions {
		...
		if suggestion.creatorID != wantCreatorID {
			t.Errorf("編集提案%dの作成者が交互になっていない", suggestion.number)
		}
	}

	for _, suggestion := range suggestions {
		// 反映された編集提案には、それがいつ行われたかが記録され、反映されて
		// いない編集提案には記録されていてはならない。画面がステータスを報告する
		// 根拠がこの記録であるため。
		if suggestion.status == model.SuggestionStatusApplied && suggestion.appliedAt == nil {
			t.Errorf("反映済みの編集提案%qに反映時刻が無い", suggestion.title)
		}
		if suggestion.status != model.SuggestionStatusApplied && suggestion.appliedAt != nil {
			t.Errorf("反映されていない編集提案%qに反映時刻が入っている", suggestion.title)
		}

		pages := listSuggestionPages(ctx, t, tx, spaces.wiki.id, suggestion.id)
		if len(pages) == 0 {
			t.Fatalf("編集提案%qが1件も提案ページを持っていない", suggestion.title)
		}
		...
		for _, suggestionPage := range pages {
			...
			if got := countSuggestionPageRevisions(ctx, t, tx, spaces.wiki.id, suggestionPage.id); got == 0 {
				t.Errorf("編集提案%qの提案ページにリビジョンが1件も無い", suggestion.title)
			}
		}
	}
}

// assertSuggestionPageShowsADiffは、提案されたページが差分として読めること、
...
func assertSuggestionPageShowsADiff(
	...
	if suggestionPage.pageRevisionID == nil {
		t.Errorf("編集提案%qの提案ページに差分の基準となるリビジョンが無い", suggestion.title)
	...
	if suggestionPage.body == base {
		t.Errorf("編集提案%qの提案ページが、基準リビジョンと同じ本文だった", suggestion.title)
	}
	...
	if suggestion.status == model.SuggestionStatusApplied {
		if page.body != suggestionPage.body {
			t.Errorf("反映済みの編集提案%qの内容が、ページに反映されていない", suggestion.title)
		}
		if got := countPageRevisions(ctx, t, tx, spaceID, suggestionPage.pageID); got < 2 {
			t.Errorf("反映済みの編集提案%qのページのリビジョンが%d件しか無い", suggestion.title, got)
		}
	...
	if page.body != base {
		t.Errorf("反映されていない編集提案%qが、ページの本文を書き換えている", suggestion.title)
	}
}
...
func TestGenerateSuggestionsShowcaseStates(t *testing.T) {
	...
	for _, spec := range specs {
		if _, ok := byTitle[spec.title]; !ok {
			t.Fatalf("編集提案%qが作られていない", spec.title)
		}
	}

	// 一覧は新しい編集提案から順に見せる。その先頭で出会う価値があるのは、固有の
	// 状態を見せるために書いた編集提案である。比較で同時刻を許しているのは、作成時刻を
	// シードが決めていないため。行を書くのは画面が呼ぶのと同じCreateであり、2件が
	...
	for _, spec := range specs {
		if byTitle[spec.title].createdAt.Before(newestOrdinary) {
			t.Errorf("編集提案%qが、通常の編集提案より前に作成されている", spec.title)
		}
	}

	// 1件の編集提案が複数のページを一度に変更し、その最後のページの名前を
	...
	if want := showcaseSpecByTitle(specs, multiPageSuggestionTitle).changedPages; len(pages) != want {
		t.Fatalf("編集提案%qの提案ページが%d件であることを期待したが%d件だった", multiPage.title, want, len(pages))
	}

	renamed := 0
	for _, suggestionPage := range pages {
		if suggestionPage.title == nil {
			t.Fatalf("編集提案%qの提案ページにタイトルが無い", multiPage.title)
		}

		page := readPageUnderSuggestion(ctx, t, tx, spaces.wiki.id, suggestionPage.pageID)
		if page.title == nil {
			t.Fatalf("編集提案%qの対象ページにタイトルが無い", multiPage.title)
		}
		if *suggestionPage.title != *page.title {
			renamed++
			if !strings.HasSuffix(*suggestionPage.title, renamedPageTitleSuffix) {
				t.Errorf("名前の変更を提案しているタイトル%qが想定の形になっていない", *suggestionPage.title)
			}
		}
	}
	if renamed != 1 {
		t.Errorf("名前の変更を提案している提案ページが1件であることを期待したが%d件だった", renamed)
	}

	// 1件の編集提案が議論を持つ。コメントは書かれた順に表示され、同じ順に採番
	// される。また2つのアカウントが交互に発言するため、スレッドが1人の独り言として
	// 読まれることがない。
	...
	if len(comments) != amt.suggestionComments {
		t.Fatalf("編集提案のコメントが%d件であることを期待したが%d件だった", amt.suggestionComments, len(comments))
	}
	for i, comment := range comments {
		position := i + 1
		if comment.number != int32(position) {
			t.Errorf("%d件目のコメントの番号が%dであることを期待したが%dだった", position, position, comment.number)
		}
		wantCreatorID := spaces.wiki.member(roleOwner).id
		if position%2 == 0 {
			wantCreatorID = spaces.wiki.member(roleCollaborator).id
		...
		if comment.creatorID != wantCreatorID {
			t.Errorf("%d件目のコメントが、交互に発言する順番どおりの投稿者になっていない", position)
		}
		if comment.body == "" {
			t.Errorf("%d件目のコメントの本文が空だった", position)
		}
	}
	for _, suggestion := range suggestions {
		...
		if got := len(listSuggestionComments(ctx, t, tx, spaces.wiki.id, suggestion.id)); got != 0 {
			t.Errorf("編集提案%qにコメントが%d件付いている", suggestion.title, got)
		}
	}

	// 1件の編集提案は編集の途中にある。編集画面が残した下書きは提案ページに
	...
	if len(editedPages) == 0 {
		t.Fatalf("編集提案%qが提案ページを持っていない", editStarted.title)
	}

	drafts := listDraftPages(ctx, t, tx, spaces.wiki.id, spaces.wiki.member(roleOwner).id)
	if len(drafts) != 1 {
		t.Fatalf("編集中の下書きが1件であることを期待したが%d件だった", len(drafts))
	}
...
// TestGenerateSuggestionsStampsTheStartedEditDraftBehindEarlierDraftsは、
...
func TestGenerateSuggestionsStampsTheStartedEditDraftBehindEarlierDrafts(t *testing.T) {
	...
	// ここで受け取る打刻は、先行するフェーズが書いた下書きを表す。編集提案の
	...
	if !drafts[0].modifiedAt.Before(earlier) {
		t.Errorf(
			"編集中の下書きの最終更新%vが、先に書かれた下書きの%vより後になっている",
...
func TestSuggestionPageBodyKeepsTheBaseAsContext(t *testing.T) {
	...
	// 差分は、ページが現在保持している内容との間で取られる。共通する行を持たない
	// 提案はページ全体の置き換えとして表示され、見出しを書き換える提案は、変更を段落の
	// 中ではなくページの見出し構成へ移してしまう。
	//
	// 段落を2行にまたがらせているのは、シードが書く本文がそうなっていないため。文は
...
`
	...
	if sharedLineCount(base, body) == 0 {
		t.Error("提案された本文に、基準と共通する行が1行も無い")
	}
}
...
func TestSuggestionPageBodyEndsTheParagraphItRewrites(t *testing.T) {
	...
	// 編集提案の対象になるページは、ページネーション用ページの生成器が書いた本文を
	// 持っている。段落の末尾以外へ文を足すと別の文の途中へ入り込み、画面では壊れた1文
	// として読まれる。
	for i, template := range bulkPageBodies {
		...
		for j, line := range lines {
			...
			if j+1 < len(lines) && strings.TrimSpace(lines[j+1]) != "" {
				t.Errorf("%d件目の本文で、書き換えた行の後に段落が続いている: %q", i+1, line)
			}
		}
		if rewritten != 1 {
			t.Errorf("%d件目の本文で書き換えられた行が%d行だった", i+1, rewritten)
		}
	}
}
...
func TestSuggestionCopyUsesJapanese(t *testing.T) {
	...
	// 期待値は、実装側のタイトルや本文から意図的に独立させる。定数やspecから
	// 導出すると、実装が英語へ戻ったときにテストも追従し、退行を検出できないため。
	// ここに英単語を残す理由は無いため、ASCIIの英字が1文字でも含まれている場合も
	// 失敗とする。
	...
	if len(showcases) != len(showcaseWants) {
		t.Fatalf("固有の状態を見せる編集提案が%d件であることを期待したが%d件だった", len(showcaseWants), len(showcases))
	}
	for i, want := range showcaseWants {
		if showcases[i].title != want.title {
			t.Errorf("%d件目の固有の編集提案のタイトルが%qであることを期待したが%qだった", i+1, want.title, showcases[i].title)
		}
	...
	if len(suggestionCommentBodies) != len(commentMarkers) {
		t.Fatalf("編集提案のコメント本文が%d件であることを期待したが%d件だった", len(commentMarkers), len(suggestionCommentBodies))
	}
	...
	for _, tt := range copies {
		for _, marker := range tt.markers {
			if !strings.Contains(tt.copy, marker) {
				t.Errorf("%sに日本語のマーカー%qが含まれていない: %q", tt.name, marker, tt.copy)
			}
		}
...
func TestSuggestionBodiesCarryNoWikilinks(t *testing.T) {
	...
	// 提案された本文にWikiリンクがあると、resolverがその名前のページを作成し、
	...
	for _, body := range bodies {
		if got := len(markup.ScanWikilinks(body, topicNameHandbook)); got != 0 {
			t.Errorf("編集提案の本文にWikiリンクが%d件含まれている: %q", got, body)
		}
	}
}
...
func TestGenerateSuggestionsRejectsTooFewOpen(t *testing.T) {
	...
	// 固有の状態を見せるために書いた編集提案は固定の一覧であるため、オープンな
...
func TestGenerateSuggestionsUpdatesAppliedEditorTimes(t *testing.T) {
	...
	// 通常のオープンな編集提案は、反映済みのものより先に作られる。その次の対象が
	// 最初の反映済み編集提案に使われる。この一括生成ページは既にroleOwnerが書いており、
	// PageEditorを新規作成せず既存のものを返すFindOrCreateの経路を通る。
	...
	if affected != 1 {
		t.Fatalf("反映対象にownerの既存ページ編集者が1件あることを期待したが%d件だった", affected)
	}
	...
	if affected != 1 {
		t.Fatalf("「ハンドブック」にownerのトピックメンバーが1件あることを期待したが%d件だった", affected)
	}
...
func TestGenerateSuggestionsExcludesPagesWithDrafts(t *testing.T) {
	...
	// 対象取得クエリが本来最初に返すページへ下書きを付ける。後続ページだけで
	...
	if count != 0 {
		t.Errorf("下書き付きページが%d件の編集提案に使われている", count)
	}
}
...
func TestGenerateSuggestionsRejectsTooFewTargets(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "編集提案の対象にできるページ") {
		t.Errorf("対象ページ不足のエラーを期待したが%vだった", err)
	}
}
...
func TestCreateSuggestionRejectsAnUnknownStatus(t *testing.T) {
	...
	// 編集提案が落ち着く先のステータスは、それぞれ書き下しておく必要がある。
	...
	if !strings.Contains(err.Error(), "未知のステータス") {
		t.Errorf("未知のステータスのエラーを期待したが%vだった", err)
	}
}
...
func TestDefaultAmountsFillBothSuggestionTabs(t *testing.T) {
	...
	// この一覧はページングしないため、件数を突き合わせる1ページあたりの件数は
	// 無い。守るべきなのは、どちらのタブも空のまま出会わないこと、決着済みのタブが
	// 編集提案の終わり方2つを両方持つこと、そしてオープンのタブが、固有の状態を
	...
	if got, least := defaultAmounts.openSuggestions, len(showcaseSuggestionSpecs()); got <= least {
		t.Errorf("オープンな編集提案%d件が、固有の状態を見せる%d件を超えることを期待した", got, least)
	}
	for _, tt := range []struct {
		name  string
		count int
		...
		if tt.count < 1 {
			t.Errorf("%sが1件以上であることを期待したが%d件だった", tt.name, tt.count)
		}
	}

	// コメントの本文は、互いに答えながら最後の1件で収まる1つのスレッドとして
	...
	if got, least := len(suggestionCommentBodies), defaultAmounts.suggestionComments; got < least {
		t.Errorf("コメントの本文が%d件しかなく、既定の%d件に足りない", got, least)
	}
}

// generateSuggestionsForTestは、編集提案の対象にできるスペース・トピック・
...
// prepareSuggestionsForTestは、編集提案の対象にできるスペース・トピック・
...
// showcaseTitlesは、固有の状態を見せるために書いた編集提案のタイトルを列挙する。
// 通常の編集提案と見分けるために使う。
...
// showcaseSpecByTitleは、指定タイトルの仕様を返す。
...
// sharedLineCountは、2つの本文に共通する行を数える。差分が変更のない文脈として
// 見せるのはこの行になる。
...
// listSuggestionsは、あるトピックの編集提案を、一覧が見せる順序で読み戻す。
...
// suggestionPageRowは、編集提案が保存している提案ページ1件。
...
// suggestionCommentRowは、編集提案に保存されたコメント1件。
...
// listSuggestionCommentsは、編集提案のコメントを、議論が見せる順序で読み戻す。
// 並び順がコメント番号に落ちるようにしているのは、同じ時刻に書かれた2件も採番された
// 順に読めるようにするため。
...
// countSuggestionPageRevisionsは、提案ページが辿ったリビジョンの件数を数える。
...
// countPageRevisionsは、ページが辿ったリビジョンの件数を数える。編集提案が反映
...
// readPageRevisionBodyは、あるリビジョン時点でページが保持していた本文を読み
...
// pageUnderSuggestionは、編集提案が変更を提案する先のページ。
type pageUnderSuggestion struct {
	title *string
	body  string
}

// readPageUnderSuggestionは、編集提案が変更を提案する先のページを読み戻す。
...
// readDraftSuggestionPageIDは、下書きが紐づいている提案ページを読み戻す。
