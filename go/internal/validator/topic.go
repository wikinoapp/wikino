...
// topicNameForbiddenCharsはトピック名が持てない文字。ページタイトルが持てない区切り文字と
// 同じもので、理由も同じである (pageTitleForbiddenCharsを参照)。
const topicNameForbiddenChars = pageTitleForbiddenChars

// TopicCreateValidatorはトピック作成のバリデーションを行う
type TopicCreateValidator struct {
	topicRepo *repository.TopicRepository
}

// NewTopicCreateValidatorはTopicCreateValidatorを生成する
...
// TopicCreateValidatorInputはバリデーションの入力パラメータ
...
// Validateはバリデーションを行い、フォームが送信した公開範囲を変換して返す。
...
// TopicUpdateValidatorはトピックの一般設定の更新のバリデーションを行う
type TopicUpdateValidator struct {
	topicRepo *repository.TopicRepository
}

// NewTopicUpdateValidatorはTopicUpdateValidatorを生成する
...
// TopicUpdateValidatorInputはバリデーションの入力パラメータ
...
func (v *TopicUpdateValidator) Validate(ctx context.Context, input TopicUpdateValidatorInput) (model.TopicVisibility, error) {
	...
	// 更新するトピック自身は一意性チェックから除く。既に持っている名前をそのままにした
...
// validateTopicFieldsは作成フォームと一般設定フォームが共有する項目を検証し、フォームが
...
// validateTopicNameはトピック名の形式を検証し、見つかったものをveに積む。
func validateTopicName(ctx context.Context, ve *model.ValidationError, name string) {
	...
	// 制御文字も拒否する。理由はページタイトルと同じで、名前を表示する場所ごとに描画のされ方が
	// 異なり、読み手が読める名前の一部にもならないためである。
	if strings.ContainsFunc(name, unicode.IsControl) {
		ve.AddField("name", i18n.T(ctx, "validation_topic_name_control_chars"))
	}

	// 先頭・末尾の空白を拒否するのはファイル名の都合ではなく名前自身の都合である。そこだけが
	// 違う2つの名前は、一覧に並んだときに同じものに見える。
