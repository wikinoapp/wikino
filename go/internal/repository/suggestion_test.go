...
func TestSuggestionRepository_Create(t *testing.T) {
	...
	t.Run("編集提案を作成できる", func(t *testing.T) {
		...
		if suggestion.ID == "" {
			t.Error("suggestion.IDが空")
		}
		if suggestion.SpaceID != spaceID {
			t.Errorf("suggestion.SpaceID = %v、期待値 = %v", suggestion.SpaceID, spaceID)
		}
		if suggestion.TopicID != topicID {
			t.Errorf("suggestion.TopicID = %v、期待値 = %v", suggestion.TopicID, topicID)
		}
		if suggestion.CreatedSpaceMemberID != spaceMemberID {
			t.Errorf("suggestion.CreatedSpaceMemberID = %v、期待値 = %v", suggestion.CreatedSpaceMemberID, spaceMemberID)
		}
		if suggestion.Title != "テスト提案" {
			t.Errorf("suggestion.Title = %v、期待値 = テスト提案", suggestion.Title)
		}
		if suggestion.Body != "テスト本文" {
			t.Errorf("suggestion.Body = %v、期待値 = テスト本文", suggestion.Body)
		}
		if suggestion.Status != model.SuggestionStatusDraft {
			t.Errorf("suggestion.Status = %v、期待値 = %v", suggestion.Status, model.SuggestionStatusDraft)
		}
		if suggestion.AppliedAt != nil {
			t.Errorf("suggestion.AppliedAt = %v、期待値 = nil", suggestion.AppliedAt)
		}
	})
}
...
func TestSuggestionRepository_FindByID(t *testing.T) {
	...
	t.Run("IDで編集提案を取得できる", func(t *testing.T) {
		...
		if suggestion.ID != suggestionID {
			t.Errorf("suggestion.ID = %v、期待値 = %v", suggestion.ID, suggestionID)
		}
		if suggestion.Title != "検索テスト提案" {
			t.Errorf("suggestion.Title = %v、期待値 = 検索テスト提案", suggestion.Title)
		}
	})
	...
	t.Run("存在しないIDはnilを返す", func(t *testing.T) {
		...
		if suggestion != nil {
			t.Errorf("FindByID() = %v、期待値 = nil", suggestion)
		}
	})
...
func TestSuggestionRepository_ListByTopicAndStatuses(t *testing.T) {
	...
	t.Run("オープンステータスの編集提案を取得できる", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("ListByTopicAndStatuses()のエラー = %v", err)
		}
		if len(suggestions) != 2 {
			t.Fatalf("len(suggestions) = %v、期待値 = 2", len(suggestions))
		}
	})
	...
	t.Run("該当なしの場合は空のスライスを返す", func(t *testing.T) {
		...
		if len(suggestions) != 0 {
			t.Errorf("len(suggestions) = %v、期待値 = 0", len(suggestions))
		}
	})
}
...
func TestSuggestionRepository_UpdateStatus(t *testing.T) {
	...
	t.Run("ステータスをオープンに更新できる", func(t *testing.T) {
		...
		if suggestion.Status != model.SuggestionStatusOpen {
			t.Errorf("suggestion.Status = %v、期待値 = %v", suggestion.Status, model.SuggestionStatusOpen)
		}
		if suggestion.AppliedAt != nil {
			t.Errorf("suggestion.AppliedAt = %v、期待値 = nil", suggestion.AppliedAt)
		}
	...
	t.Run("ステータスを反映済みに更新しapplied_atを設定できる", func(t *testing.T) {
		...
		if suggestion.Status != model.SuggestionStatusApplied {
			t.Errorf("suggestion.Status = %v、期待値 = %v", suggestion.Status, model.SuggestionStatusApplied)
		}
		if suggestion.AppliedAt == nil {
			t.Fatal("suggestion.AppliedAtがnil")
		}
	})
}
...
func TestSuggestionRepository_CountByTopicAndStatuses(t *testing.T) {
	...
	t.Run("オープンステータスの件数を取得できる", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("CountByTopicAndStatuses()のエラー = %v", err)
		}
		if count != 2 {
...
// Rails側の削除経路 (DestroyTopicJobなど) が頼るON DELETE CASCADEの
// 契約を検証する。topicsの行を直接DELETEしたとき、明示的なDELETEなしで
...
func TestSuggestionRepository_CascadeOnTopicDelete(t *testing.T) {
	...
	// 削除対象のトピック。pages.topic_idのFK (Rails由来、CASCADEなし)
	// がDELETEを妨げないよう、ページは別トピックに置く。
	...
	t.Run("トピックの行を直接DELETEすると提案ツリーも消える", func(t *testing.T) {
		// 親のtopicsの行を直接削除 (アプリケーションコードを経由しない)
		...
		if err != nil {
			t.Fatalf("DELETE topicsのエラー = %v", err)
		}
		...
		for _, c := range counts {
			...
			if err != nil {
				t.Fatalf("SELECT COUNT(*) FROM %sのエラー = %v", c.table, err)
			}
			if count != 0 {
				t.Errorf("%sの件数 = %d、期待値 = 0 (カスケード削除されていない)", c.table, count)
			}
		}

		// 別トピックのページは連鎖削除に巻き込まれないこと。
		var pageCount int
		if err := tx.QueryRowContext(
			...
			t.Fatalf("SELECT COUNT(*) FROM pagesのエラー = %v", err)
		}
		if pageCount != 1 {
			t.Errorf("pagesの件数 = %d、期待値 = 1 (ページが削除されている)", pageCount)
		}
	})
}
