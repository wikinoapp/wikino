...
// SuggestionPageRepositoryは編集提案ページリポジトリ
type SuggestionPageRepository struct {
	q *query.Queries
}

// NewSuggestionPageRepositoryはSuggestionPageRepositoryを生成する
...
// FindByIDはIDで編集提案ページを取得する (スペースIDでスコープ)
...
// ListBySuggestionIDは編集提案IDで編集提案ページ一覧を取得する
...
// UpdateSuggestionPageContentInputは編集提案ページコンテンツ更新の入力パラメータ
...
// UpdateContentは編集提案ページのコンテンツを更新する
...
// ExistsByPageIDAndOpenStatusは指定ページを参照しているオープンな編集提案が存在するかを確認する
...
// toModelはquery.SuggestionPageをmodel.SuggestionPageに変換する
...
// toModelsはquery.SuggestionPageのスライスをmodel.SuggestionPageのスライスに変換する
