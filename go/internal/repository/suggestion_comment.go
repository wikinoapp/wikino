...
// SuggestionCommentRepositoryは編集提案コメントリポジトリ
type SuggestionCommentRepository struct {
	q *query.Queries
}

// NewSuggestionCommentRepositoryはSuggestionCommentRepositoryを生成する
...
// FindByIDはIDで編集提案コメントを取得する (スペースIDでスコープ)
...
// ListBySuggestionIDは編集提案IDでコメント一覧を取得する (作成日時の昇順)
...
// CountBySuggestionIDは編集提案IDでコメント数を取得する
...
// FindByNumberは編集提案IDと番号でコメントを取得する (スペースIDでスコープ)
...
// GetNextNumberは編集提案内の次のコメント番号を取得する
...
// toModelはquery.SuggestionCommentをmodel.SuggestionCommentに変換する
...
// toModelsはquery.SuggestionCommentのスライスをmodel.SuggestionCommentのスライスに変換する
