...
func TestCardLinkDraftPage_DisplayTitle(t *testing.T) {
	...
	t.Run("下書きタイトルが設定されているときはそれを返す", func(t *testing.T) {
		...
		if got := card.DisplayTitle(ctx); got != "下書きタイトル" {
			t.Errorf("DisplayTitle() = %q、期待値 = %q", got, "下書きタイトル")
		}
	})
	...
	t.Run("下書きタイトルが空のときは公開ページのタイトルにフォールバックする", func(t *testing.T) {
		...
		if got := card.DisplayTitle(ctx); got != "公開ページタイトル" {
			t.Errorf("DisplayTitle() = %q、期待値 = %q", got, "公開ページタイトル")
		}
	})

	t.Run("下書きタイトルがnilのときは公開ページのタイトルにフォールバックする", func(t *testing.T) {
		draft := &model.DraftPage{
			Title: nil,
			Page:  &model.Page{Number: 1, Title: strPtr("公開ページタイトル")},
			Topic: &model.Topic{
	...
	t.Run("下書き・公開ページのタイトルがいずれも未設定のときは 無題 を返す", func(t *testing.T) {
		...
		if got := card.DisplayTitle(ctx); got != "無題" {
			t.Errorf("DisplayTitle() = %q、期待値 = %q", got, "無題")
		}
	})
}
...
func TestNewCardLinkDraftPage_FieldMapping(t *testing.T) {
	...
	t.Run("公開トピックはglobe-regularアイコンを設定する", func(t *testing.T) {
		draft := &model.DraftPage{
			Title: strPtr("タイトル"),
		...
		if card.TopicName != "トピックA" {
			t.Errorf("TopicName = %q、期待値 = %q", card.TopicName, "トピックA")
		}
		...
		if card.PageNumber != 7 {
			t.Errorf("PageNumber = %d、期待値 = %d", card.PageNumber, 7)
		}
	...
	t.Run("非公開トピックはlock-regularアイコンを設定する", func(t *testing.T) {
		draft := &model.DraftPage{
			Title: strPtr("タイトル"),
...
func TestNewCardLinkDraftPages(t *testing.T) {
	...
	if cards[0].TopicName != "T1" || cards[1].TopicName != "T2" {
		t.Errorf("トピック名が一致しない: %q、%q", cards[0].TopicName, cards[1].TopicName)
	}
}
...
func TestNewCardLinkDraftPagesWithoutSpace(t *testing.T) {
	...
	for i, card := range cards {
		// スペース名は空 (編集画面ではスペースラベルを表示しない)
		if card.SpaceName != "" {
			t.Errorf("cards[%d].SpaceName = %q、期待値 = 空", i, card.SpaceName)
		}
		// リンク生成に必要なSpaceIdentifierは保持する
		if card.SpaceIdentifier == "" {
			t.Errorf("cards[%d].SpaceIdentifierが空", i)
		}
	}
	if cards[0].TopicName != "T1" || cards[1].TopicName != "T2" {
		t.Errorf("トピック名が一致しない: %q、%q", cards[0].TopicName, cards[1].TopicName)
