...
func (h *Handler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	// UseCaseを実行
	_, err = h.removeSuggestionPageUsecase.Execute(ctx, usecase.RemoveSuggestionPageInput{
	...
	if err != nil {
		h.handleDeleteError(w, r, err, spaceIdentifier, suggestionNumber)
		return
	}

	// フラッシュメッセージを設定してリダイレクト
	h.flashMgr.SetSuccess(w, i18n.T(ctx, "flash_suggestion_page_removed"))
...
func (h *Handler) handleDeleteError(w http.ResponseWriter, r *http.Request, err error, spaceIdentifier model.SpaceIdentifier, suggestionNumber model.SuggestionNumber) {
	ctx := r.Context()
	...
	slog.ErrorContext(ctx, "編集提案ページの削除に失敗", "error", err)
	http.Error(w, "Internal Server Error", http.StatusInternalServerError)
}
