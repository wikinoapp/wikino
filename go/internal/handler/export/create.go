...
// Createはエクスポートを開始します (POST /s/{space_identifier}/settings/exports)。
...
// handleCreateErrorは拒否された開始に応答する。既にエクスポート中のスペースでは、閲覧者を
// それを占めているエクスポートへ送る。その画面が何が起きているかを伝え、完了を追う場所でもある。
// その間は画面が開始ボタンを隠すため、ここに到達するのは2つのリクエストが競合したときである。
