...
// SetupSpaceWithMemberはテスト用にuser / space / spaceMemberを一括作成するヘルパー。
//
// prefixはテスト内でユニークな識別子を作るために使用する (email / atname / space identifier
// に同じ値を埋め込み、t.Parallel() 同士の衝突を避ける)。複数のビルダーを束ねた一括セットアップ
// であり、Wikinoのテストでは「最小単位の公開可能なリソース構成」を組むケースが多いため、
