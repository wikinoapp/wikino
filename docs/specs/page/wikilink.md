...
Wikiリンクにより自動作成された未公開かつ本文が空のページは、他のページのタイトルリネーム時に論理削除される場合がある。例えば、ユーザーが `[[共同編集機能]]` と入力する途中で `[[共同編集]]` まで書いた段階で自動保存が実行されると「共同編集」というページが自動作成される。後に「共同編集機能」ページを「共同編集」にリネームしようとした場合、自動作成された未公開ページが競合するため、システムはその競合ページを論理削除（タイトルをページ自身のIDに変更し `discarded_at` を設定）してリネームを許可する。

論理削除の対象は未公開かつ本文が空のページ（`published_at IS NULL AND body = ''`）のみであり、本文が入力されているページは保護される。

詳細は [ページ編集 仕様書](edit.md) の「タイトルリネーム時の未公開ページ競合解消」を参照。
