...
Go 版で実装されており、全ユーザーが利用できる。Rails 版に対応する機能はない。
...
編集提案の閲覧可否は、所属するトピックの公開・非公開設定に従う。

- 公開トピックの編集提案は、未ログイン・非メンバーでも閲覧可能（一覧・詳細・変更差分画面）
- 非公開トピックの編集提案は、スペースメンバー（`topic:read` スコープ保持者）のみ閲覧可能。未ログイン・非メンバーには 404 を返す
- 閲覧可能であっても、書き込み操作（作成・反映・クローズ・コメント・編集）にはスペースメンバー権限が必要
...
| アクション                                     | 必要なスコープ                        | 追加条件                                           |
| ---------------------------------------------- | ------------------------------------- | -------------------------------------------------- |
| 閲覧（一覧・詳細・変更差分画面）               | 非公開トピックの場合のみ `topic:read` | 公開トピックは未ログイン・非メンバーも閲覧可能     |
...
- タイトル横に「新規編集提案」ボタンを配置（`@components.MainTitle` の `Actions`）。`CanCreateSuggestion` が true の場合のみ表示
- 閲覧の可否は[閲覧のアクセス制御](#閲覧のアクセス制御)を参照
