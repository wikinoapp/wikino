...
権限はスコープベースの `Authorizer` インターフェース（`MemberPolicy` / `GuestPolicy`）で判定される。スペースメンバーの有効スコープは、`space_members.scopes` と `topic_members.scopes` の和集合を含意ルールで展開した結果である。

- 公開トピックは全員が閲覧できる
- 非公開トピックは `topic:read` スコープを持つスペースメンバーのみが閲覧できる。非メンバーやスコープを持たないユーザーがアクセスした場合は 404 を返す
- `page:write` スコープを持つスペースメンバーは「新規ページ」ボタンからページ作成画面に遷移できる
- `topic:write` スコープを持つスペースメンバーはドロップダウンメニューから「トピック設定」画面へのリンクにアクセスできる
...
5. 権限チェック: `Authorizer.CanShowTopic` で判定。トピックが非公開の場合、`topic:read` スコープを持つスペースメンバーでなければ 404
