...
- [@docs/tasks/2_todo/page-show-go-migration.md](../2_todo/page-show-go-migration.md) - ページ表示画面のGo移行（本タスクが前提）
- [@docs/tasks/2_todo/page-ogp-meta.md](../2_todo/page-ogp-meta.md) - ページのOGPメタタグ設定（本タスクが前提）
- ~~[@docs/tasks/2_todo/wikilink-completion.md](../2_todo/wikilink-completion.md)~~ - Wikiリンク補完（本タスクに統合済み）
- ~~[@docs/tasks/2_todo/topic-member-timestamp.md](../2_todo/topic-member-timestamp.md)~~ - TopicMemberのタイムスタンプ更新（本タスクに統合済み）
- ~~[@docs/tasks/2_todo/wikilink-auto-creation.md](../2_todo/wikilink-auto-creation.md)~~ - Wikiリンクによるページ自動作成（本タスクに統合済み）
...
- トピックの変更が可能（トピックセレクターで変更。選択肢はスペースメンバーが参加しているトピックのみ）
- トピックの変更は自動保存のたびにDraftPageに反映され、公開時にPageに反映される
...
- `GET /s/:space_identifier/page_locations?q=:keyword` - ページロケーション検索（Wikiリンク補完用）
...
- ページ編集画面（タイトル・本文・トピックセレクター、CodeMirrorエディタ）
...
以下は別タスクに委譲し、初回スコープには含めない（各タスクリストは「関連タスク」セクションを参照）:
...
│   ├── edit.go         # GET /s/:space_identifier/pages/:page_number/edit
│   ├── update.go       # PATCH /s/:space_identifier/pages/:page_number（公開）
│   └── validator.go    # ページ公開のバリデーション（タイトル重複チェック等）
├── draft_page/
│   ├── handler.go      # Handler構造体と依存性
│   └── update.go       # PATCH /s/:space_identifier/pages/:page_number/draft_page（自動保存）
└── page_location/
    ├── handler.go      # Handler構造体と依存性
    └── index.go        # GET /s/:space_identifier/page_locations?q=:keyword（Wikiリンク補完用）
```
...
- `topic.go` — FindBySpaceAndNumber, ListActiveBySpace, FindBySpaceAndNames, ListJoinedBySpaceMember（スペースメンバーが参加しているトピック一覧。編集画面のトピックセレクター用）
...
- `update_draft_page.go` — 下書き自動保存（find_or_create + Markdownレンダリング + 添付ファイルフィルター + Wikiリンク解析・自動ページ作成 + 更新。トピック変更にも対応: `topic_number`パラメータで指定されたトピックに変更、未指定時はPageの現在のトピックを維持）
...
- `internal/usecase/update_draft_page.go` — 自動保存時のWikiリンク処理を追加
...
- Rails版ではTurbo Streamで自動保存のたびにフッターを動的更新しているが、Go版ではTurboを使用しないため、ページ初回表示時のみフッターにリンク一覧・バックリンクを表示する
- 自動保存時のフッター動的更新は含めない
...
Rails版ではTurbo Streamで部分更新（保存時刻表示、フッターのリンク一覧更新）を行っている。Go版ではTurboを使用しないため、JSONレスポンスで返す:

```json
{
  "modified_at": "2024-01-01T12:34:56Z"
}
```

フロントエンドのJavaScriptで保存時刻のテキストを更新する。リンク一覧・バックリンクの動的更新は初めない（ページ初回表示時のみ表示）。
...
`GET /s/:space_identifier/page_locations?q=:keyword`
...
Rails版では`TopicPolicy`パターン（Guest/Member/Admin/Ownerの4種類のポリシークラス）で実装されている。Go版ではRails版と同等のアクセス制御ロジックをシンプルな構造体として実装する。

`internal/policy/topic.go`に`TopicPolicy`構造体を配置する:

```go
type TopicPolicy struct {
	spaceMember *model.SpaceMember
	topicMember *model.TopicMember // nilの場合あり（非トピックメンバー）
	topic       *model.Topic
}

// CanUpdatePage はページの編集・公開が可能かを判定する
func (p *TopicPolicy) CanUpdatePage() bool {
	if p.spaceMember == nil || !p.spaceMember.Active {
		return false
	}
	// スペースオーナーは全トピックのページを編集可能
	if p.spaceMember.Role == model.SpaceMemberRoleOwner {
		return true
	}
	// トピックメンバーは所属トピックのページを編集可能
	return p.topicMember != nil
}

// CanUpdateDraftPage は下書きの更新が可能かを判定する（CanUpdatePageと同じルール）
func (p *TopicPolicy) CanUpdateDraftPage() bool {
	return p.CanUpdatePage()
}
```
...
| ------------------ | ---------- | ---------- | ------------------------------------------------------------ |
| スペースオーナー   | ✅         | ✅         | アクティブなスペースメンバーであること                       |
| トピックAdmin      | ✅         | ✅         | アクティブなスペースメンバー かつ トピックメンバーであること |
| トピックMember     | ✅         | ✅         | アクティブなスペースメンバー かつ トピックメンバーであること |
| 非トピックメンバー | ❌         | ❌         | トピックメンバーでない場合は編集不可                         |
| 非スペースメンバー | ❌         | ❌         | スペースメンバーでない場合は編集不可                         |

**ハンドラーでの使用**:

各ハンドラー（edit.go, draft_page/update.go, page/update.go）で以下の流れで認可チェックを行う:

1. `SpaceMemberRepository.FindActiveBySpaceAndUser`でスペースメンバーを取得
2. `TopicMemberRepository.FindBySpaceMemberAndTopic`でトピックメンバーを取得（存在しない場合はnil）
3. `TopicPolicy`を生成し、`CanUpdatePage()`/`CanUpdateDraftPage()`で権限チェック
...
- [ ] **1-2**: [Go] SpaceMember モデル・クエリ・リポジトリの追加
...
- [ ] **1-3**: [Go] Topic モデル・クエリ・リポジトリの追加
  ...
  - `internal/repository/topic.go` を追加（FindBySpaceAndNumber, ListActiveBySpace, FindBySpaceAndNames, ListJoinedBySpaceMember）
  - `internal/testutil/topic_builder.go` を追加
  - FindBySpaceAndNames: スペース内で指定されたトピック名の一覧に一致するトピックを取得する（Wikiリンク解析時のトピック一括検索用）
  - ListJoinedBySpaceMember: スペースメンバーが参加しているトピックの一覧を取得する（編集画面のトピックセレクター用。`topic_members`テーブルとJOIN）
  - 想定ファイル数: 実装 4 ファイル（+ sqlc生成 1 ファイル）, テスト 2 ファイル
  - 想定行数: 実装 ~150 行, テスト ~120 行
  - 依存: 1-1

- [ ] **1-4**: [Go] TopicMember モデル・クエリ・リポジトリの追加
...
- [ ] **1-5**: [Go] TopicPolicy の追加
  - `internal/policy/topic.go` を追加（TopicPolicy構造体、CanUpdatePage, CanUpdateDraftPage メソッド）
  - スペースオーナーは全トピックのページを編集可能
  - トピックメンバー（admin/member）は所属トピックのページを編集可能
  - 非トピックメンバーは編集不可
  - 想定ファイル数: 実装 1 ファイル, テスト 1 ファイル
  - 想定行数: 実装 ~50 行, テスト ~100 行
  - 依存: 1-2, 1-4
...
- [ ] **2-2**: [Go] DraftPage モデル・クエリ・リポジトリの追加
...
- [ ] **2-3**: [Go] PageRevision モデル・クエリ・リポジトリの追加
...
- [ ] **2-4**: [Go] PageEditor モデル・クエリ・リポジトリの追加
...
- [ ] **2-5**: [Go] PageAttachmentReference モデル・クエリ・リポジトリの追加
...
- [ ] **2-6**: [Go] Attachment モデル・クエリ・リポジトリの追加
...
- [ ] **3-2**: [Go] 添付ファイルフィルター（AttachmentFilter）の追加
...
- [ ] **3-3**: [Go] 添付ファイルID抽出・アイキャッチ画像ID抽出ロジックの追加
...
- [ ] **3-4**: [Go] Wikiリンクパース・HTML変換ロジックの追加
...
- [ ] **5-1**: [Go] ページ編集ハンドラーとテンプレートの追加
  - `internal/handler/page/edit.go` を追加（DraftPage存在確認、フォーム表示）
  - `internal/templates/pages/page/edit.templ` を追加（タイトル入力、本文テキストエリア、トピックセレクター、CSRFトークン、公開ボタン）
  - 翻訳ファイルにページ編集関連のメッセージを追加
  - `cmd/server/main.go` にルーティングを登録
  ...
  - **トピックセレクターの選択肢**: スペースメンバーが参加しているトピックのみを表示する（Rails版の`selectable_topics`に相当。`space_member_record.topic_records`を使用）
...
- [ ] **5-2**: [Go] ページ編集画面のリンク一覧表示の追加
  - `internal/handler/page/edit.go` を更新（リンク一覧のデータ取得を追加）
  - `internal/templates/pages/page/edit.templ` を更新（フッターにリンク一覧セクションを追加）
  - `internal/templates/components/link_list.templ` を追加（リンク一覧コンポーネント。ページタイトルとリンクを表示）
  - `internal/viewmodel/link_list.go` を追加（リンク一覧のビューモデル）
  - 翻訳ファイルにリンク一覧関連のメッセージを追加
  - リンク一覧: DraftPage存在時はDraftPageの`linkedPageIds`、なければPageの`linkedPageIds`を基に表示
  - 自動保存時のフッター動的更新は含めない（ページ初回表示時のみ）
  - ページネーションは含めない
  - 想定ファイル数: 実装 4 ファイル, テスト 1 ファイル
  - 想定行数: 実装 ~120 行, テスト ~70 行
  - 依存: 2-1, 5-1

- [ ] **5-3**: [Go] ページ編集画面のバックリンク一覧表示の追加
...
- [ ] **5-4**: [Go] ページロケーション検索APIの追加（Wikiリンク補完用）
  - `internal/handler/page_location/handler.go` を追加（Handler構造体と依存性）
  - `internal/handler/page_location/index.go` を追加（GET /s/:space_identifier/page_locations?q=:keyword）
...
- [ ] **6-1**: [Go] 下書き自動保存ユースケースの追加
  - `internal/usecase/update_draft_page.go` を追加（find_or_create方式でDraftPageを取得・作成し、Markdownレンダリング + 添付ファイルフィルター + Wikiリンク解析・自動ページ作成・HTML変換後に更新）
  - **find_or_createの競合対策**: DraftPageのユニーク制約（`space_member_id + page_id`）違反時はリトライする（Rails版の`rescue RecordNotUnique; retry`に相当）
  - **トピック変更対応**: `topic_number`パラメータが指定されている場合はそのトピックに変更、未指定の場合はPageの現在のトピックを維持する（Rails版の`DraftPages::UpdateService`に相当）
  - Wikiリンク処理: bodyからWikiリンクをパース → トピック検索 → リンク先ページの自動作成 → DraftPageのlinkedPageIdsを更新 → bodyHTML内のWikiリンクを`<a>`タグに変換
  - 想定ファイル数: 実装 1 ファイル, テスト 1 ファイル
  - 想定行数: 実装 ~180 行, テスト ~200 行
  - 依存: 2-2, 3-1, 3-2, 3-4

- [ ] **6-2**: [Go] 下書き自動保存ハンドラーの追加
...
- [ ] **7-2**: [Go] ページ公開ハンドラー・バリデーターの追加
  ...
  - `cmd/server/main.go` にルーティングを登録
  - `TopicPolicy.CanUpdatePage()`によるトピックアクセス制御（権限がない場合は404）
  - **バリデーション失敗時**: 編集画面をリンク一覧・バックリンク一覧付きで再表示する（HTTPステータス422）
  - **公開成功時**: フラッシュメッセージ（`messages.pages.saved`相当）を設定し、ページ表示画面（`/s/{space_identifier}/pages/{page_number}`）にリダイレクト
  - 想定ファイル数: 実装 4 ファイル, テスト 2 ファイル
  - 想定行数: 実装 ~150 行, テスト ~180 行
  - 依存: 1-5, 5-1, 7-1

- [ ] **7-3**: [Go] ページ公開時の添付ファイル参照同期・アイキャッチ画像設定の追加
...
- [ ] **8-1**: [Go] Playwright E2Eテスト基盤のセットアップ
...
- [ ] **8-2**: [Go] CodeMirrorエディタと自動保存JSの追加 + E2Eテスト（タブインデント・リスト続行）
...
- [ ] **8-4**: [Go] エディタのドラッグ&ドロップ・ペーストアップロードの追加 + E2Eテスト（ファイルアップロード・ペースト）
...
- [ ] **8-5**: [Go] Wikiリンク補完フロントエンドの追加 + E2Eテスト
  ...
  - API呼び出し: `fetch(/s/${spaceIdentifier}/page_locations?q=${keyword})`でページロケーションを取得
...
- [ ] **9-1**: [Go] ページ編集関連パスのリバースプロキシ設定の追加
  ...
  - 想定ファイル数: 実装 1 ファイル, テスト 1 ファイル
  - 想定行数: 実装 ~30 行, テスト ~80 行
  - 依存: 5-1, 5-4, 6-2, 7-2, 8-2, 8-4, 8-5
