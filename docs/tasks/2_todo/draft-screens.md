...
- [下書き 仕様書](../specs/draft_page/overview.md)（タスク完了後に作成予定）
...
下書き詳細画面と下書き一覧画面を新規に実装する。これらの画面は、ユーザーが自分の下書きを管理し、編集提案の作成フローへの入り口として機能する。

**下書き詳細画面** (`GET /s/:space_id/topics/:topic_id/draft`): 特定のトピック内の自分の下書きページを表示する。下書きページのタイトル・編集履歴（DraftPageRevision）・差分を確認できる。ここからページを選択して「編集提案する...」操作へ遷移できる。
...
- 前提: [@docs/tasks/2_todo/draft-page-revision.md](draft-page-revision.md) - DraftPageRevision の実装
- 前提: [@docs/tasks/2_todo/diff-component.md](diff-component.md) - 差分表示コンポーネント
...
- ユーザーは特定のトピック内の自分の下書きページを一覧表示できる
- 各下書きページのタイトルが表示される
- タイトルの左にチェックボックスがあり、選択すると「編集提案する...」ボタンが表示される
- タイトルを選択すると下書きの編集履歴（DraftPageRevision一覧）が表示される
- 編集履歴を選択すると差分が表示される
...
UI設計の詳細は [@docs/tasks/2_todo/edit-suggestion.md](edit-suggestion.md) の「UI設計」セクションを参照。
...
- [ ] **N-1**: 仕様書の作成・更新
  - `docs/specs/draft_page/overview.md` に仕様書を作成する
