...
ユーザーが明示的にDraftPage（下書き）を破棄できる機能を実装する。破棄すると自分のDraftPageが削除され、公開版（Pageの現在の内容）に戻る。

Rails版では「キャンセル」リンクがページ表示に戻るのみで、DraftPageは削除されない（DraftPageは公開時にのみ削除される）。Go版では明示的な破棄操作を追加し、不要な下書きをユーザーがクリーンアップできるようにする。
...
- ユーザーはDraftPageを明示的に破棄できる
- 破棄すると自分のDraftPageが削除される
- 公開版（Pageの現在の内容）に戻る
...
1. 自分のDraftPageを削除する
2. 公開版（Pageの現在の内容）の表示にリダイレクトする

- 他のユーザーのDraftPageには影響しない
- 認可チェック: DraftPageの所有者であることを確認
...
- [ ] **N-1**: 仕様書の作成・更新
  - `docs/specs/page/edit.md` の仕様書に下書き破棄機能の内容を追記する
