...
- [ ] **8b-1**: [Go] Datastarの導入とページ編集画面のリンク一覧表示の追加
  - `package.json` にDatastar関連パッケージを追加
  - esbuildの設定を更新（Datastarのバンドル）
  - `internal/handler/page/edit.go` を更新（リンク一覧のデータ取得を追加）
  - `internal/templates/pages/page/edit.templ` を更新（フッターにDatastarのシグナルを使用したリンク一覧セクションを追加）
  ...
  - 下書き自動保存のレスポンスを更新し、Datastarのイベントでリンク一覧のHTMLフラグメントを返す
  - ページネーションは含めない
  - 想定ファイル数: 実装 6 ファイル, テスト 1 ファイル
  - 想定行数: 実装 ~200 行, テスト ~100 行
  - 依存: 2-1, 5-1, 6-2, 8-2
