...
type Page struct {
	...
	PublishedAt               *time.Time // nilなら未公開（一覧等に表示されない）
...
3. 新規作成されたページは空のbody/bodyHtmlと空のlinkedPageIdsを持ち、`published_at`は`NULL`とする
...
#### `pages.published_at`の扱い

`published_at`はページが公開済みかどうかを示すカラムであり、一覧表示・検索のフィルタリング条件（`published_at IS NOT NULL`）として使用される。ただし、ページ編集画面のリンク一覧・バックリンク一覧では`published_at`によるフィルタリングは行わない（Rails版の`available`スコープに準拠）。ページ詳細画面のリンク一覧・バックリンク一覧では`published_at IS NOT NULL`でフィルタリングする。

**`published_at`が設定されるタイミング**:

- **ページ公開時**（`PublishPageUsecase`）: 公開日時として現在時刻を設定する。既に`published_at`が設定されている場合も最新の公開日時で上書きする

**`published_at`が`NULL`であるケース**:

- **Wikiリンクによる自動ページ作成時**（`CreateLinkedPage`）: `NULL`のままとする。リンク記法の入力中に自動保存が実行されると不完全なページ名でページが作成されるため、`published_at`に日時を入れると不完全なページが一覧等に表示されてしまう。自動作成されたページは、ユーザーが内容を入力して公開するまで未公開状態を維持する
...
- 同じスペース内の未廃棄（`discarded_at IS NULL`）のページを表示する
- ページ編集画面では`published_at`によるフィルタリングは行わない（Wikiリンクで自動作成された未公開ページもリンク一覧に表示するため。Rails版の`available`スコープに準拠）
- ページ詳細画面では`published_at IS NOT NULL`でフィルタリングする（今後の実装時に別途クエリを追加する）

**バックリンク一覧**:

- `pages`テーブルの`linked_page_ids`カラムにこのページのIDが含まれるページの一覧を取得・表示する（常にPageのデータを基にする）
- 同じスペース内の未廃棄のページを表示する
- ページ編集画面では`published_at`によるフィルタリングは行わない（Rails版の`available`スコープに準拠）
- ページ詳細画面では`published_at IS NOT NULL`でフィルタリングする（今後の実装時に別途クエリを追加する）
...
- [x] **2-1**: [Go] Page モデル・クエリ・リポジトリの追加
  ...
  - FindByIDs: `linkedPageIds`に含まれるIDのページを取得（リンク一覧表示用。同スペース・未廃棄のページのみ）
  - FindBacklinkedByPageID: `linked_page_ids`カラムに指定ページIDが含まれるページを取得（バックリンク一覧表示用。同スペース・未廃棄のページのみ）
...
- [x] **8b-1**: [Go] ページ編集画面のリンク一覧表示の追加
  ...
  - **サブタスク**:
    ...
    - [x] **8b-1c**: 下書き自動保存レスポンスとJSの更新
      ...
      - 想定ファイル数: 2 ファイル
    - [x] **8b-1d**: Datastar Go SDKの追加とリンク一覧SSEエンドポイントの作成
...
- [x] **11a-1**: [Go] ユーザーの下書きページ一覧を取得する SQL クエリとリポジトリメソッドを実装する
...
- [x] **11a-2**: [Go] 下書き一覧を Datastar SSE でサイドバーに非同期表示する
  - サイドバーのテンプレートに Datastar の `data-on-intersect` 属性を使用し、サイドバーが画面に表示されたタイミングで SSE リクエストを発行する
...
- [x] **13-1**: [Go] Wikiリンクによる自動ページ作成時に`published_at`を`NULL`にする
  - **背景**: `CreateLinkedPage`クエリでは`published_at`に`modified_at`と同じ値を設定しているが、リンク記法を入力してから正式なページ名を入力するまでの間に自動保存が実行されると、不完全なページ名のページが作成される。`published_at`に日時が入っていると不完全なページが表から見えてしまう
  - **対応**: `CreateLinkedPage`クエリの`published_at`を`NULL`に変更する
  - 修正対象: `go/db/queries/pages.sql` の `CreateLinkedPage` クエリ
  - sqlc再生成: `make sqlc-generate`
  - テスト: 既存のWikiリンク自動ページ作成テストで`published_at`が`NULL`であることを確認するアサーションを追加
  - 想定ファイル数: 実装 1 ファイル（+ sqlc生成 1 ファイル）, テスト 1 ファイル
  - 想定行数: 実装 ~5 行（変更）, テスト ~10 行
  - 依存: 12-1

- [x] **13-2**: [Go] ページ編集画面のリンク一覧・バックリンク一覧で未公開ページが表示されない不具合を修正
  - **背景**: リンク関連の全SQLクエリに`published_at IS NOT NULL`条件が含まれていたため、Wikiリンクで自動作成された未公開ページ（`published_at = NULL`）がページ編集画面のリンク一覧・バックリンク一覧に表示されなかった。Rails版のページ編集画面では`available`スコープ（`kept.topics_kept.not_trashed`）を使用しており`published_at`をチェックしていない
  - **対応**: ページ編集画面で使用するリンク関連の8つのSQLクエリから`published_at IS NOT NULL`条件を削除
  - **注意**: ページ詳細画面のリンク一覧・バックリンク一覧では`published_at IS NOT NULL`でフィルタリングが必要。ページ詳細画面のGo移行時に`published_at`フィルタ付きのクエリを別途追加する
  - 修正対象: `go/db/queries/pages.sql`（FindPagesByIDs, FindBacklinkedPagesByPageID, FindLinkedPagesPaginated, CountLinkedPages, FindBacklinkedPagesPaginated, CountBacklinkedPages, FindBacklinkedPagesForTargets, CountBacklinkedPagesForTargets）
  - sqlc再生成: `make sqlc-generate`
  - テスト: 既存テストの期待値を修正（非公開ページも結果に含まれるように変更）
  - 依存: 13-1

- [x] **13-3**: [Go] リンク一覧でリンク元カードとバックリンクカードを視覚的に区別する
  - **背景**: リンクセクションでは、A. 本文にリンク記法で書いたページのカードと、B. そのページへのバックリンクのカードが同じスタイルで表示されており、どれが本文からのリンク先かわかりにくい
  - **対応**: A（本文からのリンク先）のカードに左ボーダー（`border-l-4 border-l-primary`）を付けて「起点」であることを視覚的に示す
  - 修正対象:
    - `go/internal/viewmodel/page.go` の `CardLinkPage` 構造体に `Primary bool` フィールドを追加
    - `go/internal/templates/components/card_link_page.templ` で `Primary` が `true` の場合に左ボーダーを適用
    - `go/internal/templates/components/link_list.templ` で `CardLinkPage` 呼び出し時に `Primary` を設定
  - 想定ファイル数: 実装 3 ファイル
  - 想定行数: 実装 ~15 行（変更）
  - 依存: 12-1

### フェーズ 14: フィーチャーフラグからホワイトリストへの移行（全ユーザー展開）

- [ ] **14-1**: [Go] フィーチャーフラグからホワイトリストへの移行（全ユーザー展開）
...
### フェーズ 15: Rails版の実装の削除

- [ ] **15-1**: [Rails] ページ編集・公開関連のコントローラー・サービス・フォーム・ビューの削除
