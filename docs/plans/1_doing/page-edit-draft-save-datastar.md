# ページ編集画面の下書き保存を Datastar SSE に移行 作業計画書
...
ページ編集画面の下書き自動保存において、保存時刻の表示を JavaScript による DOM 操作から Datastar SSE によるサーバードリブン UI に移行する。

現在の実装では、`saveAsDraft` 関数が `fetch` で PATCH リクエストを送り、JSON レスポンスから `modified_at` を取得して JavaScript 側で `toLocaleTimeString` を使って時刻文字列を生成し、DOM を直接操作して表示している。この方式では時刻のフォーマットや表示の DOM 構造が JavaScript 側に分散している。

Datastar SSE を活用することで、下書き保存時刻の DOM 構造とフォーマットを templ テンプレート側に一元管理できるようになり、JavaScript 側が薄くなる。既にリンク一覧の更新で使用している `draft-autosaved` カスタムイベント + Datastar `@get()` のパターンを拡張する形で実装する。
...
自動保存は CodeMirror の `EditorView.updateListener` から発火する命令的な処理のため、Datastar の `@patch()` は使えない。そこで以下の設計とする：

1. **JS の `fetch` PATCH は維持する**が、レスポンスを JSON → 204 No Content に変更
2. 保存成功後に `draft-autosaved` カスタムイベントを dispatch（既存の仕組み）
3. このイベントで Datastar の `@get()` が `page_link_list` SSE エンドポイントを呼び出す（既存の仕組み）
4. **`page_link_list` ハンドラーを拡張**し、リンク一覧に加えて「下書き保存時刻」のフラグメントも送信する

これにより、1 回の SSE レスポンスでリンク一覧と保存時刻の両方を更新できる。
...
[JS: fetch PATCH /go/s/{id}/pages/{num}/draft_page]
    ↓ (204 No Content)
[JS: window.dispatchEvent("draft-autosaved")]
    ↓
[Datastar: data-on:draft-autosaved__window="@get('/go/s/{id}/pages/{num}/link_list')"]
    ↓ (SSE)
[Server: 2つのフラグメントを送信]
    ├─ Fragment 1: #page-link-list (リンク一覧)
    └─ Fragment 2: #page-draft-saved-at (保存時刻)
```
...
#### GET `/go/s/{space_identifier}/pages/{page_number}/link_list` の変更

- **Before**: SSE フラグメント 1 つ（`#page-link-list`）
- **After**: SSE フラグメント 2 つ（`#page-link-list` + `#page-draft-saved-at`）

### テンプレート設計

#### 新規コンポーネント: `components/draft_saved_time.templ`

`time.Time` を受け取り、「下書き保存: **HH:MM**」形式で表示する。
...
保存/キャンセルボタンの下に `#page-draft-saved-at` 要素を追加し、`data-markdown-editor-saved-at` 属性を削除する。
...
description = "下書き保存時刻の表示"
other = "下書き保存: <span class=\"font-bold\">{{.Time}}</span>"

# en.toml
[page_edit_draft_saved_time]
description = "Draft saved time display"
other = "Draft saved: <span class=\"font-bold\">{{.Time}}</span>"
```
...
| `go/internal/templates/components/draft_saved_time.templ`          | 新規     | 下書き保存時刻コンポーネント                                    |
| `go/internal/templates/pages/page/edit.templ`                      | 修正     | `#page-draft-saved-at` 追加、`data-markdown-editor-saved-at` 削除 |
| `go/internal/handler/page_link_list/show.go`                       | 修正     | 保存時刻フラグメントの追加送信                                  |
| `go/internal/handler/draft_page/update.go`                         | 修正     | 204 No Content に変更、JSON レスポンス削除                      |
| `go/web/markdown-editor/markdown-editor.ts`                        | 修正     | JSON パース・DOM 操作の削除、`savedAtEl` の削除                 |
...
- 自動保存は CodeMirror の `updateListener` から命令的に呼ばれるため、Datastar の `@patch()` 属性が使えない
- `fetch` で PATCH を送る部分は残す必要があり、SSE レスポンスを受け取っても Datastar が自動的に処理しない
- 既存の `draft-autosaved` イベント + `page_link_list` SSE エンドポイントのパターンを拡張する方がシンプル
...
- 1 つの SSE レスポンスで複数のフラグメントを送信できる Datastar の機能を活用すれば、1 回のリクエストで両方を更新できる
...
- [ ] **1-1**: [Go] 下書き保存を Datastar SSE に移行
  - i18n 翻訳の追加（`page_edit_draft_saved_time` を ja.toml、en.toml に追加）
  - `components/draft_saved_time.templ` の新規作成
  - `pages/page/edit.templ` の修正（`#page-draft-saved-at` 追加、`data-markdown-editor-saved-at` 削除）
  - `handler/page_link_list/show.go` の拡張（保存時刻フラグメントの追加送信）
  - `handler/draft_page/update.go` の簡素化（204 No Content に変更）
  - `web/markdown-editor/markdown-editor.ts` の簡素化（JSON パース・DOM 操作・`savedAtEl` の削除）
  - **想定ファイル数**: 約 8 ファイル（実装 7 + 自動生成 1）
  - **想定行数**: 約 60 行（実装 60 行、テストは既存テストで担保）
