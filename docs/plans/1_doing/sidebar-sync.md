...
- サイドバーのフッターリンク（ヘルプ・ロードマップ・コミュニティ・© Wikino）を削除し、Go版と同じフッター（利用規約・プライバシーポリシー・著作権表示）をレイアウトに追加する
...
| フッターリンク               | なし（レイアウトにフッターあり）               | あり（ヘルプ・ロードマップ・コミュニティ）                    |
...
- [x] **5-1**: [Rails] サイドバーのフッターリンクを削除し、Go版と同じフッターをレイアウトに追加する
  - `SidebarComponent` からフッターリンク（ヘルプ・ロードマップ・コミュニティ・© Wikino）を削除
  - `Layouts::Column1Component` にGo版 `footer.templ` と同じフッターを追加
  - フッターの内容: 利用規約リンク（`/terms`）、プライバシーポリシーリンク（`/privacy`）、著作権表示（© 2025-2026 Wikino）
  - Go版と同じHTML構造・スタイリングで実装
  - I18n翻訳キーを追加（ja, en）
  - **想定ファイル数**: 約 4 ファイル（実装 4 + テスト 0）
  - **想定行数**: 約 40 行（実装 40 行 + テスト 0 行）
