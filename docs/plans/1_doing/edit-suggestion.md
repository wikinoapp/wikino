...
3. [下書き機能のアップデート](../2_todo/draft-update.md) - DraftPageRevision・下書き一覧画面（1が前提）
4. [差分表示コンポーネント](diff-component.md) - テキスト間の差分表示（1が前提）
5. [タイトル変更時のWikiリンク自動書き換え](title-change-link-rewrite.md) - `[[旧タイトル]]` → `[[新タイトル]]`（1が前提）
