...
- 「編集提案 #12345 のページを編集中です」というメッセージを表示
...
- [x] **2-3**: [Go] トピック詳細画面に「編集提案」タブを追加
  ...
  - **フィーチャーフラグ対応**: `go_suggestion` フラグが有効な場合のみタブを表示する。トピック詳細画面自体は `/suggestions` 配下ではないため、リバースプロキシではなくアプリケーション内でフラグをチェックする
...
- [x] **10-1**: [Go] ページ編集画面の「下書き保存して編集提案を作成する...」アクション
  ...
  - **フィーチャーフラグ対応**: `go_suggestion` フラグが有効な場合のみドロップダウンアクションを表示する
  - **想定ファイル数**: 実装 2, テスト 1
  - **想定行数**: 実装 約80行, テスト 約60行

- [x] **10-2**: [Go] 下書き一覧画面の「編集提案する...」ボタン
