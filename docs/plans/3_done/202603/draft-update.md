...
- [x] **2-2**: [Go] 編集画面に「下書き保存」ボタンを追加
  ...
  - 現時点ではその場に留まる動作。下書き一覧画面（3-2）完了後にリダイレクト先を変更する（3-3）
  - 想定ファイル数: 実装 7, テスト 1
  - 想定行数: 実装 120, テスト 80
  - 依存: 2-1
...
- [x] **3-1**: [Go] 下書き一覧用のクエリとリポジトリメソッド追加
...
- [x] **3-2**: [Go] 下書き一覧画面の ViewModel・テンプレート・ハンドラー
...
- [x] **3-3**: [Go] 下書き保存後に下書き一覧画面へリダイレクト
  - `internal/handler/draft_page/create.go` のレスポンスをリダイレクトに変更（`GET /drafts` へ）
  - 下書き保存の目的は「保存して編集を終え、編集提案を作成する」導線のため
  - 想定ファイル数: 実装 1, テスト 1
  - 想定行数: 実装 10, テスト 20
  - 依存: 3-2

- [x] **3-4**: [Go] 下書き保存ボタンの修正（フォーム送信 + Usecase拡張）
  - `ManualSaveDraftPageUsecase` を拡張し、フォームから受け取ったタイトル・本文でDraftPageをfind_or_create・更新した上でリビジョンを作成する
  - ハンドラー (`create.go`) でフォームからタイトル・本文を受け取るように変更
  - 依存: 3-3

- [x] **3-5**: [Go] 下書き保存ボタンを `formaction` 方式に変更し、`draft_page_revision` リソースに分離
  - 公開用フォームには `_method=PATCH` のhidden inputがあるため、下書き保存も同じPATCHメソッドに統一する
  - 別 `<form>` + `onsubmit` JS による値同期を廃止し、公開フォーム内に `formaction` 属性付きの下書き保存ボタンを配置する
  - `formaction` によりsubmit先URLだけが変わり、フォーム内の `title`, `body` 等はそのまま送信される
  - ハンドラーガイドの命名規則（PATCHは `update.go`）に準拠するため、新リソース `draft_page_revision` を作成
  - ルーティング: `PATCH /s/{space_identifier}/pages/{page_number}/draft_page_revision`
  - 変更ファイル:
    - `internal/handler/draft_page_revision/handler.go` - 新規Handler構造体
    - `internal/handler/draft_page_revision/update.go` - PATCHハンドラー（旧 `draft_page/create.go` を移動）
    - `internal/handler/draft_page_revision/main_test.go` - TestMain
    - `internal/handler/draft_page_revision/update_test.go` - テスト（旧 `draft_page/create_test.go` を移動）
    - `internal/handler/draft_page/handler.go` - `manualSaveDraftPageUC` の依存を削除
    - `internal/handler/draft_page/create.go` - 削除
    - `internal/handler/draft_page/create_test.go` - 削除
    - `internal/templates/pages/page/edit.templ` - 別フォームを廃止し `formaction` ボタンに変更
    - `internal/templates/path.go` - `PageDraftPageRevisionPath` に改名
    - `internal/handler/page/edit.go` - 新パスヘルパーを使用
    - `cmd/server/main.go` - ルーティング変更・ハンドラー初期化変更
  - 依存: 3-4
