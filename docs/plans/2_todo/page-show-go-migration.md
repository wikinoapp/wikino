...
- [ページ表示 仕様書](../specs/page/show.md)（タスク完了後に作成予定）
...
ページ表示画面（GET /s/:space_identifier/pages/:page_number）をRailsからGoに移行する。公開済みページの`body_html`をレンダリングし、トピックの公開設定に基づくアクセス制御を行う。
...
- ユーザーはGo版でページを表示できる
- 公開済みページの`body_html`をレンダリングできる
- ログインユーザーがスペースメンバーの場合は編集ボタンを表示する
- トピックの公開設定に基づくアクセス制御を行う
...
- [ ] **N-1**: 仕様書の作成・更新
  - `docs/specs/page/show.md` に仕様書を作成する
