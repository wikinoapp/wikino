...
# inside to_prepare (the same hook active_storage_extensions.rb uses). Because
...
# to_prepare 内で行う (active_storage_extensions.rb と同じフック)。discard_on は呼ぶ
# たびにハンドラを追加し、to_prepare はコードリロードのたびに再実行されるため、同じ
# ハンドラを二重登録しないようガードする。
