...
module SentryConfig
  ...
  # Resolve the traces sample rate from a raw value (typically returned by
  # `Wikino.config.sentry_traces_sample_rate`). Falls back to `default` when
  ...
  #
  # [Ja] サンプリングレートの生値 (通常は `Wikino.config.sentry_traces_sample_rate`)
