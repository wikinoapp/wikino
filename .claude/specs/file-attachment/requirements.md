...
- アップロード可能なファイル形式
  - 実行可能ファイル（.exe, .bat, .sh, .app など）は除外
  - ホワイトリスト方式で許可する形式を管理
...
- サムネイル生成
  - 小（150x150）、中（300x300）、大（800x800）の3サイズを自動生成
  - 遅延読み込み（Intersection Observer API使用）でパフォーマンス最適化
- 画像クリックで拡大表示（ライトボックス機能）
- EXIF情報に基づく自動回転補正
