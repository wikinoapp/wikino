...
- [ ] Active Storageのインストールと設定
  ...
  - マイグレーションファイルの確認と実行
- [ ] ストレージサービスの設定
  - 開発環境: MinIOの設定を`config/storage.yml`に追加
  - 本番環境: Cloudflare R2の設定を`config/storage.yml`に追加
- [ ] 環境別のストレージ設定
  - `config/environments/development.rb`でMinIOを使用
...
### 2.1 モデル・レコード
- [ ] Attachment::Recordの実装
  - ActiveStorageとの関連付け
  - スコープ: by_space, by_user, recent
- [ ] AttachmentReference::Recordの実装
  - ページとの関連付け
  - 表示順序の管理
...
1. インフラ・基盤設定（Active Storage、データベース）
2. バックエンドのファイルアップロードAPI
3. フロントエンドのアップロード機能
4. ファイル管理画面
5. セキュリティ強化
6. テスト作成
