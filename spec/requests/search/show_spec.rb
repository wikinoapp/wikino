...
RSpec.describe "GET /search", type: :request do
  ...
  let!(:space_membership) { create(:space_membership_record, user_record:, space_record:) }
  ...
  it "有効な検索キーワードがある場合、検索が実行されること" do
    ...
    get search_path, params: { keyword: "テスト" }
  ...
  it "検索結果がない場合、適切なメッセージが表示されること" do
    get search_path, params: { keyword: "存在しないキーワード" }
  ...
  it "無効な検索キーワードの場合、エラーメッセージが表示されること" do
    get search_path, params: { keyword: "a" }
  ...
  it "他のユーザーのスペースのページは検索結果に含まれないこと" do
    other_user_record = create(:user_record)
    other_space_record = create(:space_record)
    other_topic_record = create(:topic_record, space_record: other_space_record)
    create(:space_membership_record, user_record: other_user_record, space_record: other_space_record)
    create(:page_record, 
    ...
    get search_path, params: { keyword: "他のユーザー" }
