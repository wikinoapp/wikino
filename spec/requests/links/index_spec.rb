...
RSpec.describe "POST /s/:space_identifier/pages/:page_number/links", type: :request do
  it "トピックが削除されているとき、そのトピックに投稿されたページは表示されないこと" do
    ...
    linked_page_record = create(:page_record, :published, space_record:, topic_record:, title: "テストページ")
    page_record = create(:page_record, space_record:, topic_record:, linked_page_ids: [linked_page_record.id])
