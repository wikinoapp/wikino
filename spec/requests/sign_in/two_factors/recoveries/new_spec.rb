...
RSpec.describe "GET /sign_in/two_factor/recovery/new", type: :request do
  it "既にログインしているとき、ホーム画面にリダイレクトすること" do
    ...
    get "/sign_in/two_factor/recovery/new"
  ...
  it "ログインしていない & 2FAが有効 & `pending_user_id` が有効なとき、リカバリーコード入力画面が表示されること" do
    user_record = create(:user_record, :with_password)
    create(:user_two_factor_auth_record, :enabled, user_record:)
    set_session(pending_user_id: user_record.id)
    ...
    expect(response.body).to include("二要素認証のリカバリー")
  end
end
