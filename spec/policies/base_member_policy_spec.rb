...
RSpec.describe BaseMemberPolicy do
  ...
  describe "#in_same_space?" do
    it "同じスペースに所属している場合はtrueを返すこと" do
      ...
      expect(policy.in_same_space?(space.id)).to be(true)
    end
    ...
    it "異なるスペースIDの場合はfalseを返すこと" do
      ...
      expect(policy.in_same_space?(other_space.id)).to be(false)
    end
    ...
    it "space_member_recordがnilの場合はfalseを返すこと" do
      ...
      expect(policy.in_same_space?("any_space_id")).to be(false)
    end
  end
  ...
  describe "#joined_topic?" do
    it "トピックに参加している場合はtrueを返すこと" do
      ...
      expect(policy.joined_topic?(topic.id)).to be(true)
    end
    ...
    it "トピックに参加していない場合はfalseを返すこと" do
      ...
      expect(policy.joined_topic?(topic.id)).to be(false)
    end
    ...
    it "space_member_recordがnilの場合はfalseを返すこと" do
      ...
      expect(policy.joined_topic?("any_topic_id")).to be(false)
    end
  end
